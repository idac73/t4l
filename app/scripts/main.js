...
if(!(isMobile)) {
  window.onresize = function(event) {
    ...
    $('#home .container').css({'margin-top':''+ m +'px','margin-bottom':''+ m +'px' });    
    $.each($('section'), function() {
      $(this).animate({'min-height':$(window).height()}, 500);
        var m = (($(window).height() - $(this).height()) / 4) > 0 ? (($(window).height() - $(this).height()) / 4) : 0;
        $('.container', this).css({'margin-top':''+ m +'px','margin-bottom':''+ m +'px' });
    });  
  };
  
  window.onload = function(event) {
    $('#home').animate({height:$(window).height()}, 500);
    var m = (($(window).height() - $('#home').height()) / 4) > 0 ? (($(window).height() - $('#home').height()) / 4) : 0;
    $('#home .container').css({'margin-top':''+ m +'px','margin-bottom':''+ m +'px' });     
    $.each($('section'), function() {
      $(this).animate({'min-height':$(window).height()}, 500);
        var m = (($(window).height() - $(this).height()) / 4) > 0 ? (($(window).height() - $(this).height()) / 4) : 0;
...
(function () {
  var contactFormUtils = {
    ...
    clearForm: function () {
      $('#contactForm .glyphicon').removeClass('glyphicon-check').addClass('glyphicon-unchecked').css({color: ''});
